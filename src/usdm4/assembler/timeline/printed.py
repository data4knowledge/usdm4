...
def read_cycle(text: str | None) -> CycleNumber | CycleRange | None:
    """Read a printed cycle: a single cycle or a range (issue 66). A trailing
    parenthetical qualifier is ignored; anything else unread is ``None``."""
    if is_blank(text):
        return None
    text = _QUALIFIER_RE.sub("", normalise(text)).strip()
    match = _CYCLE_RE.match(text)
    if match:
        return CycleNumber(int(match.group("n")))
    match = _CYCLE_RANGE_RE.match(text)
    if not match:
        return None
    start = int(match.group("a"))
    if match.group("open"):
        return CycleRange(start, None)
