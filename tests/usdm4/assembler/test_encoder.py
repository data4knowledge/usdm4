...
# ---------------------------------------------------------------------------
# Element-aware normalisation reporting (added alongside Encoder.phase /
# document_status / etc.). These paths emit typed entries on the shared
# Errors log when the source value differs from the canonical form.
# Use a real Errors instance so we can inspect ._items directly.
# ---------------------------------------------------------------------------


NORMALIZATION_TAG = "m11_normalization_record"


class TestEncoderToDateNormalisation:
    """to_date with ``element`` set emits a normalisation record."""

    @pytest.fixture
    def encoder(self):
        from simple_error_log.errors import Errors as RealErrors

        return Encoder(Mock(spec=Builder), RealErrors())

    def _records(self, encoder):
        return [
            i for i in encoder._errors._items if i.error_type == NORMALIZATION_TAG
        ]

    def test_element_none_suppresses_record(self, encoder):
        # Backward compat: callers that don't pass element see no record
        # regardless of input.
        encoder.to_date("15 Nov 2023")
        assert self._records(encoder) == []

    def test_iso_source_suppresses_record(self, encoder):
        # Source is already ISO-8601 canonical — the helper's equivalence
        # check suppresses.
        encoder.to_date("2023-11-15", element="Version Date")
        assert self._records(encoder) == []

    def test_non_iso_source_emits_coerced_record(self, encoder):
        encoder.to_date("15 Nov 2023", element="Version Date")
        records = self._records(encoder)
        assert len(records) == 1
        assert records[0].extra == {
            "element": "Version Date",
            "source": "15 Nov 2023",
            "normalised": "2023-11-15",
            "kind": "coerced",
        }

    def test_unparseable_source_emits_unmapped_record(self, encoder):
        encoder.to_date("not a date", element="Version Date")
        records = self._records(encoder)
        assert len(records) == 1
        assert records[0].extra["kind"] == "unmapped"
        assert records[0].extra["source"] == "not a date"

    def test_empty_source_with_element_emits_unmapped(self, encoder):
        encoder.to_date("", element="Version Date")
        records = self._records(encoder)
        assert len(records) == 1
        assert records[0].extra["kind"] == "unmapped"

    def test_whitespace_only_source_emits_unmapped(self, encoder):
        encoder.to_date("   ", element="Version Date")
        records = self._records(encoder)
        assert len(records) == 1
        assert records[0].extra["kind"] == "unmapped"
...
class TestEncoderToBooleanNormalisation:
    """to_boolean with ``element`` set emits a normalisation record."""
    ...
    def test_element_none_suppresses_record(self, encoder):
        encoder.to_boolean("y")
        assert self._records(encoder) == []

    def test_canonical_yes_suppresses_record(self, encoder):
        encoder.to_boolean("Yes", element="Original Protocol Indicator")
        assert self._records(encoder) == []

    def test_canonical_no_suppresses_record(self, encoder):
        encoder.to_boolean("No", element="Original Protocol Indicator")
        assert self._records(encoder) == []

    def test_case_variant_suppresses_record(self, encoder):
        # "YES" is case-insensitive equivalent of "Yes"; the normalisation
        # helper's _values_equivalent treats them as equal.
        encoder.to_boolean("YES", element="Original Protocol Indicator")
        assert self._records(encoder) == []

    def test_y_variant_emits_coerced_record(self, encoder):
        encoder.to_boolean("y", element="Original Protocol Indicator")
        records = self._records(encoder)
        assert len(records) == 1
        assert records[0].extra["kind"] == "coerced"
        assert records[0].extra["source"] == "y"
        assert records[0].extra["normalised"] == "Yes"

    def test_true_variant_emits_coerced_record(self, encoder):
        encoder.to_boolean("true", element="Original Protocol Indicator")
        records = self._records(encoder)
        assert len(records) == 1
        assert records[0].extra["normalised"] == "Yes"

    def test_one_variant_emits_coerced_record(self, encoder):
        encoder.to_boolean("1", element="Original Protocol Indicator")
        records = self._records(encoder)
        assert len(records) == 1
        assert records[0].extra["source"] == "1"
        assert records[0].extra["normalised"] == "Yes"

    def test_unrecognised_source_emits_unmapped_record(self, encoder):
        encoder.to_boolean("maybe", element="Original Protocol Indicator")
        records = self._records(encoder)
        assert len(records) == 1
        assert records[0].extra["kind"] == "unmapped"
        assert records[0].extra["source"] == "maybe"
        # Unmapped falls back to False → "No".
        assert records[0].extra["normalised"] == "No"

    def test_none_source_emits_unmapped_record(self, encoder):
        encoder.to_boolean(None, element="Original Protocol Indicator")
        records = self._records(encoder)
        assert len(records) == 1
        assert records[0].extra["kind"] == "unmapped"
